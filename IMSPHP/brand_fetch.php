...
foreach($result as $row)
{
    ...
    */
    ...
    $sub_array[] = $status;
    $sub_array[] = '<button type="button" name="update" id="'.$row["brandID"].'" class="btn btn-warning btn-xs update">Update</button>';
    $sub_array[] = '<button type="button" name="delete" id="'.$row["brandID"].'" class="btn btn-danger btn-xs delete" data-status="'.$row["brandID"].'">Delete</button>';
    $data[] = $sub_array;
}
...
{
    $statement = $connect->prepare('SELECT * FROM brands');
    $statement->execute();
